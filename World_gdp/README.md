...
This unit test in python file `test_world_gdp.py` validates the GDP preprocessing pipeline

- `def setUp(self)`
  - Builds a sample dataframe with wide-format GDP data for India and Brazil (country, code, indicator, and yearly columns 1980–1981)

- `test_filter_and_convert_to_long`
  - Uses `world_gdp.filter` to keep selected countries and years greater than or equal to 1980, then `convert_to_long` to reshape to long format. Checks that:
    - Only the specified country (India) remains
    - Year columns 1980 and 1981 exist, 1979 does not
    - Long-format data has exactly the columns `Country Name`, `Year`, `GDP`
    - The number of rows equals `countries_after_filter` × `number_of_year_columns`

- `test_sort_and_fix_types`
  - Verifies that:
    - `Year` is integer
    - `GDP` is numeric
    - Output is sorted by `Country Name` then `Year`
