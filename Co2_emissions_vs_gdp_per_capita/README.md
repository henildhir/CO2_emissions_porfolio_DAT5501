...
This unit test in file `test_co2_gdp.py` verifies data filtering and regression logic for the CO2 vs GDP analysis

- `def setUp(self)`
  - Builds a small sample DataFrame with multiple countries, years, GDP per capita, and CO2 per capita. This synthetic dataset is reused across tests

- `test_filter_10_years_data_handling`
  - Uses `filter_10_years` to keep only the most recent 10 years and returns min/max year

  Checks:
  - `Year` column is numeric
  - Returned `min_year` and `max_year` match the filtered data
  - Older year (2000) is correctly removed

- `test_regression_values_reasonable`
  - Calls `regression(x, y)` and validates the regression outputs:
  - Correlation coefficient r has magnitude 1.0 up to 4 decimal places (perfect linear relation in sample data)
  - Predicted values `y_ha`t have same length as grid `x_grid`
  - Confidence interval bounds `ci_lower` and `ci_upper` match x_grid length and contain only finite numbers, as does `y_hat`
  - Together these tests confirm correct time filtering, country selection, and that regression and confidence interval calculations are numerically consistent and well-formed
